...
type FormState = Omit<MembreEquipe, 'id' | 'created_at'>
const EMPTY: FormState = { nom:'', role:'', initiales:'', couleur:'green', ordre:0 }
...
const MemberForm = React.memo(({ initial, onSave, onCancel, saving }: {
  initial: FormState
    ...
    <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.1)', borderRadius:12, padding:20, display:'flex', flexDirection:'column', gap:14 }}>
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
        <div>
          ...
          <input style={inputStyle} value={form.nom} onChange={set('nom')} placeholder="Dr. Nom Prénom" autoFocus/>
        </div>
        <div>
          <label style={labelStyle}>Rôle / Spécialité</label>
          <input style={inputStyle} value={form.role} onChange={set('role')} placeholder="Médecin Généraliste"/>
        </div>
        <div>
          <label style={labelStyle}>Initiales</label>
          <input style={inputStyle} value={form.initiales} onChange={set('initiales')} placeholder="NP" maxLength={3}/>
        </div>
        <div>
          <label style={labelStyle}>Couleur avatar</label>
          <select style={{ ...inputStyle, appearance:'none' as const }} value={form.couleur} onChange={set('couleur')}>
...
const AdminEquipe: React.FC = () => {
  ...
  const handleSave = useCallback(async (form: FormState, id?: string) => {
    setSaving(true)
    if (id) {
      const { data } = await supabase.from('equipe').update(form).eq('id', id).select().single()
      if (data) setMembres(prev => prev.map(m => m.id === id ? data : m))
      setEditing(null)
    } else {
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:24 }}>
      ...
      {loading ? <Loader/> : (
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {membres.map(m => (
            <div key={m.id} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:12, padding:'16px 18px' }}>
              {editing === m.id ? (
                <MemberForm
                  initial={{ nom:m.nom, role:m.role, initiales:m.initiales, couleur:m.couleur, ordre:m.ordre }}
                  onSave={(f) => handleSave(f, m.id)}
                ...
                <div style={{ display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
                  ...
                  <span style={{ width:44, height:44, borderRadius:'50%', flexShrink:0, background: m.couleur === 'green' ? '#2D6147' : '#C8973A', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'Cormorant Garamond, serif', fontSize:'1rem', fontWeight:600, color:'#fff' }}>
                    {m.initiales}
                  </span>
                  ...
                  <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                    <button onClick={() => setEditing(m.id)} style={{ ...btnSm, background:'rgba(255,255,255,0.06)', color:'rgba(255,255,255,0.6)' }}><Pencil size={14}/></button>
          ...
          {membres.length === 0 && <p style={{ padding:'32px 0', textAlign:'center', color:'rgba(255,255,255,0.3)', fontSize:'0.88rem' }}>Aucun membre.</p>}
        </div>
      )}
    </div>
  )

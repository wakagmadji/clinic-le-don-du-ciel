...
const FALLBACK: Slide[] = [
  { id:'1', url:'https://images.unsplash.com/photo-1631217868264-e5b90bb7e133?w=800&q=80', alt:'Médecin consultant un patient', ordre:1 },
  { id:'2', url:'https://images.unsplash.com/photo-1579684385127-1ef15d508118?w=800&q=80', alt:'Équipe médicale professionnelle', ordre:2 },
  { id:'3', url:'https://images.unsplash.com/photo-1559757148-5c350d0d3c56?w=800&q=80', alt:"Laboratoire d'analyses médicales", ordre:3 },
]
...
const Hero: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'carousel' }, load)
      .subscribe()
  ...
  return (
    <section id="accueil" style={{ minHeight:'100vh', display:'flex', flexDirection:'column', paddingTop:68, position:'relative' }}>
      ...
      <div id="hero-inner" style={{ display:'flex', flexDirection:'column', flex:1 }}>
        ...
        {/* RIGHT — Carousel */}
        <div id="hero-right" style={{ background:'var(--green)', display:'flex', alignItems:'center', justifyContent:'center', padding:0, position:'relative', overflow:'hidden', minHeight:400 }}
          onMouseEnter={() => setPaused(true)}
          onMouseLeave={() => setPaused(false)}
        >
          {slides.map((slide, i) => (
            <img key={slide.id} src={slide.url} alt={slide.alt}
          ...
          <div style={{ position:'absolute', inset:0, background:'rgba(13,31,23,0.5)', zIndex:1 }}/>
          ...
          <button className="carousel-btn" onClick={prev} style={{ position:'absolute', left:16, top:'50%', transform:'translateY(-50%)', zIndex:3, width:40, height:40, borderRadius:'50%', background:'rgba(255,255,255,0.15)', border:'none', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', transition:'background 0.2s' }}>
            <ChevronLeft size={20}/>
          </button>
          <button className="carousel-btn" onClick={next} style={{ position:'absolute', right:16, top:'50%', transform:'translateY(-50%)', zIndex:3, width:40, height:40, borderRadius:'50%', background:'rgba(255,255,255,0.15)', border:'none', color:'#fff', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', transition:'background 0.2s' }}>
            <ChevronRight size={20}/>
          </button>
          <div style={{ position:'absolute', bottom:24, left:'50%', transform:'translateX(-50%)', zIndex:3, display:'flex', gap:8 }}>
            {slides.map((_, i) => (
              <button key={i} className="dot" onClick={() => setCurrent(i)}
